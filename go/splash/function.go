...
func (f *MyFunction) Handle(res http.ResponseWriter, req *http.Request) {
	...
	// request for '/' returns the index.html
	if p == "/" {
		p = "./index.html"
	} else if strings.HasSuffix(p, ".css") || strings.HasSuffix(p, ".png") {
		// add . for the local files in current directory we want to serve
		p = path.Join(".", p)
	}
